...
// #include "/home/namrata/klee/lib/Module/Passes.h"
...
// @SKLEE
namespace klee{
UseDef* globalUseDef = new UseDef();
}
// @SKLEE
//------------------------------------------------------------------------------
...
public:
  ...
  std::string getTestFilename(const std::string &suffix, unsigned id, const char* error);
...
std::string KleeHandler::getTestFilename(const std::string &suffix, unsigned id, const char* error) {
  std::stringstream filename;
  filename << "error-" << error << std::setfill('0') << std::setw(6) << id << '.' << suffix;
  return filename.str();
}
// @SKLEE
//------------------------------------------------------------------------------
...
// @SKLEE
      if(errorMessage){
          if (!kTest_toFile(&b, getOutputFilename(getTestFilename("ktest", id, errorSuffix)).c_str())) {
...
int main(int argc, char **argv, char **envp) {
  ...
  // system("/home/namrata/klee/tools/klee/run.sh");
  ...
  // UseDef obj;
  // globalUseDef->setClassUseDef(mainModule);
  // obj.writeUseDef();

  // for(auto func = mainModule->begin(); func != mainModule->end(); ++func){

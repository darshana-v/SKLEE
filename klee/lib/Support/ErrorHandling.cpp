...
static const char *conWarningPrefix = "Contract Warning";
static const char *conErrorPrefix = "Contract Error";
...
static void klee_vfmessage(FILE *fp, const char *pfx, const char *msg,
  ...
  if (modifyConsoleColor) {
    ...
    if (shouldSetColor(pfx, msg, conErrorPrefix))
    ...
    //contrac warnings
      ...
      if (shouldSetColor(pfx, msg, conWarningPrefix))
...
void klee::contractError(const char* msg, ...)
{
    va_list ap;
    va_start(ap, msg);
    klee_vmessage(conErrorPrefix, false, msg, ap);
    va_end(ap);
}

void klee::contractWarning(const char* msg, ...)
{
    va_list ap;
    va_start(ap, msg);
    klee_vmessage(conWarningPrefix, false, msg, ap);
    va_end(ap);
}

int klee::typeOverflow = 0;

void klee::overflowWarning(const char* msg, ...){
    // typeOverflow = 1;
    va_list ap;
    va_start(ap, msg);
    klee_vmessage(conWarningPrefix, false, msg, ap);
...
void klee::setOverflow(unsigned type){
    typeOverflow = type;
}

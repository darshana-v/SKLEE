...
#define addDNR(name, handler) { name, \
  ...
  add("_ZN6int256C1Ev", handleInt256, true),
  add("uint256::uint256", handleUint256, true)
...
void SpecialFunctionHandler::handleAddOverflow(ExecutionState &state,
  ...
  //                                Executor::Overflow);
    ...
    if(std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){
        ...
        executor.writeTestCase(state, os.str().c_str(), "AddOverflow");
    }
}
...
void SpecialFunctionHandler::handleSubOverflow(ExecutionState &state,
  ...
  if(std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){
      std::string TmpStr;
      llvm::raw_string_ostream os(TmpStr);
      os << "Subtraction Overflow:" << funcName << "(";
      for (unsigned i = 1; i < arguments.size(); i++) {
          os << arguments[i];
          // arguments[i]->print(os);
          // handlePrintExpr(state, target, arguments);
          if (i != arguments.size()-1)
            os << ", ";
      }
      os << ") at " << state.pc->getSourceLocation();
      ...
      klee::contractError("%s", os.str().c_str());
      buggyInst.push_back(i);
      executor.writeTestCase(state, os.str().c_str(), "SubOverflow");
  }
}
...
void SpecialFunctionHandler::handleMulOverflow(ExecutionState &state,
  ...
  if(std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){
      std::string TmpStr;
      llvm::raw_string_ostream os(TmpStr);
      os << "Multiplication Overflow:" << funcName << "(";
      for (unsigned i = 1; i < arguments.size(); i++) {
          os << arguments[i];
          // arguments[i]->print(os);
          // handlePrintExpr(state, target, arguments);
      ...
      klee::contractError("%s", os.str().c_str());
      buggyInst.push_back(i);
      executor.writeTestCase(state, os.str().c_str(), "MultOverflow");
  }
}
...
void SpecialFunctionHandler::handleDivRemOverflow(ExecutionState &state,
  ...
  //                                Executor::Overflow);
     ...
     if(std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){
         std::string TmpStr;
         llvm::raw_string_ostream os(TmpStr);
         os << "DivisionRemainder Overflow:" << funcName << "(";
         for (unsigned i = 1; i < arguments.size(); i++) {
             os << arguments[i];
             // arguments[i]->print(os);
             // handlePrintExpr(state, target, arguments);
         ...
         klee::contractError("%s", os.str().c_str());
         buggyInst.push_back(i);
         executor.writeTestCase(state, os.str().c_str(), "DivOverflow");
     }
}

void SpecialFunctionHandler::handleInt256(ExecutionState &state,
                           KInstruction *target,
                           std::vector<ref<Expr> > &arguments) {

}

void SpecialFunctionHandler::handleUint256(ExecutionState &state,
                           KInstruction *target,
                           std::vector<ref<Expr> > &arguments) {

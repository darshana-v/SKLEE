...
extern std::vector<llvm::Instruction*> buggyInst;

// extern ContractData* thisContract;
// extern ContractData copyContract;
...
public:
  ...
  void checkSendCall(ExecutionState &state);
  void checkProdigal(llvm::Function* f, ExecutionState& state, std::vector< ref<Expr> > &arguments);
  void checkSuicidal(llvm::Function* f, ExecutionState& state, std::vector< ref<Expr> > &arguments);
  int checkSuicidalFunc(llvm::Function* currFunc, llvm::CallInst* callInst, llvm::Function* calledFunc);

  void checkGreedy(llvm::Function* f, ExecutionState& state, std::vector< ref<Expr> > &arguments);
  std::string getDebugVar(llvm::Instruction* inst);
  bool checkTransferFunc(llvm::Function* f);
  std::string getFunctionName(llvm::Function* f);
  bool checkUncheckedSend(llvm::Function* f, ExecutionState &state, std::vector< ref<Expr> > &arguments);
  int checkStateUseBranch(llvm::Function* f, std::queue<llvm::BasicBlock*> q, llvm::Instruction* sendInst);
  int checkStateUse(llvm::Function* f, llvm::BasicBlock* block, llvm::Instruction* sendInst);

  void checkTOD();
  void checkTxOrigin(llvm::Instruction *currInst, ExecutionState& state);
  void checkBlockhash(llvm::Instruction *currInst, ExecutionState& state);
  void checkTimestamp(llvm::Instruction *currInst, ExecutionState& state);
  void checkFallback();

  void addUsers(llvm::Value* var, std::list<llvm::Value*> &classVar);
  std::list<llvm::Value*> getAllUsers(llvm::Value* var);

  void checkTransfer(llvm::Instruction* currInst, ExecutionState& state);
  void checkMapRead(llvm::Function* f, ExecutionState& state);
  void checkOverflow(llvm::Instruction* currInst, ExecutionState& state);

  void printValue(llvm::Value* v);
  void printVector(std::vector<llvm::Value*> vec);
  void printList(std::list<llvm::Value*> li);

  void writeTestCase(ExecutionState &state, const char* message, const char* error);
};

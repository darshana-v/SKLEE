...
std::vector<llvm::Instruction*> buggyInst;
std::vector<llvm::Function*> buggyFunctions;
ContractData newContract;
std::string callLoc;
bool firstCallLoc = true;
// std::sstream failProperty;
std::string filename = "/home/namrata/klee/tools/klee/properties.txt";
std::ofstream failProperty(filename, std::ios::out | std::ios::app);
...
UseDef* useDefObj = new UseDef();
std::map<Function*, std::list<Value*>> allClassVariables;
std::vector<std::string> contractFunctions;
...
void printFunction(FunctionData* function){
    outs() << "Name:" << function->name << "\n";
    
    for(unsigned i = 0; i < function->arguments.size(); ++i)
    {
        outs() << function->arguments[i].first << " " << function->arguments[i].second << "\n";
    }
    outs() << "retType:" << function->returnType << "\n";
    outs() << "isPure:" << function->isPure << "\n";
    outs() << "isPublic:" << function->isPublic << "\n";
    outs() << "isView:" << function->isView << "\n";
    outs() << "isPayable:" << function->isPayable << "\n";
    outs() << "hasTransferFunc:" << function->hasTransferFunc << "\n";
    outs() << "block:" << function->block << "\n";
    outs() << "args:" << function->args << "\n";
}

void printContract(ContractData contract)
{
    outs() << contract.name << "\n";
    for(auto itr = contract.stateVariables.begin(); itr != contract.stateVariables.end(); ++itr)
        outs() << itr->second << " " << itr->first << "\n";

    for(auto itr = contract.functions.begin(); itr != contract.functions.end(); ++itr)
        printFunction(*itr);

    outs() << contract.hasFallback;
}
...
// @SKLEE
  ...
  newContract = contract.readContract();
  useDefObj->setClassUseDef(m, newContract.name);
  allClassVariables = useDefObj->getFunctionClassVariables();

  for(auto itr = newContract.functions.begin(); itr != newContract.functions.end(); ++itr){
      contractFunctions.push_back((*itr)->name);
      outs() << (*itr)->name << "\n";
  }
...
void Executor::executeCall(ExecutionState &state,
  ...
  if(!f->isIntrinsic() && !f->hasInternalLinkage()){
    std::string name = getFunctionName(f);
    // checkAddressTypecast(f, state, arguments);
    if(std::find(contractFunctions.begin(), contractFunctions.end(), name) != contractFunctions.end()){        
      if(std::find(buggyFunctions.begin(), buggyFunctions.end(), f) == buggyFunctions.end()){
        // checkUncheckedSend(f, state, arguments);
        checkProdigal(f, state, arguments);
        checkSuicidal(f, state, arguments);
        checkGreedy(f, state, arguments);
        // checkMapRead(f, state);
      }
    }
  }
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
    ...
    if(getFunctionName(currFunc) == "main")
        getDebugVar(i);

    std::string name = getFunctionName(currFunc);
    
    if(std::find(contractFunctions.begin(), contractFunctions.end(), name) != contractFunctions.end()){
        if(currFunc && !currFunc->isIntrinsic() && !currFunc->hasInternalLinkage()){
            if(std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){
                checkTxOrigin(i, state);
                checkTimestamp(i, state);
                checkOverflow(i, state);
                checkTransfer(i, state);
                checkBlockhash(i, state);

                if(std::find(buggyFunctions.begin(), buggyFunctions.end(), currFunc) == buggyFunctions.end()){
                    checkUncheckedSend(currFunc, state);
                    checkMapRead(currFunc, state);
                }
            }
        }
    }
  ...
  case Instruction::Call: {
    ...
    checkAddressTypecast(i, currFunc, f, state);
    if((getFunctionName(f).find("operator-") != std::string::npos || getFunctionName(f).find("operator+") != std::string::npos
        || getFunctionName(f).find("operator*") != std::string::npos) && firstCallLoc){

            callLoc = state.pc->getSourceLocation();
            firstCallLoc = false;
    }
    // @SKLEE
    // -----------------------------------
    ...
    if (f) {
      ...
      if(typeOverflow > 0 && std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){

        std::string tempName = getFunctionName(currFunc);
        Function* tempFunc = currFunc;
        // while(std::find(newContract.functions.begin(), newContract.functions.end(), tempName) != newContract.functions.end() || tempName.find("main") != std::string::npos){
        if(CallInst* call = dyn_cast<CallInst>(i)){
            tempFunc = call->getCaller();
            if(tempFunc != NULL)
                tempName = getFunctionName(tempFunc);
        }
        // }
        ...
        switch(typeOverflow){
            case 1: os << "Addition ";break;
            case 2: os << "Subtraction ";break;
            case 3: os << "Multiplication ";break;
            case 4: os << "Division ";break;
        }
        os << "Overflow: " << tempName << "(";
        os << ") at " << callLoc;
        klee::contractError("%s", os.str().c_str());
        buggyInst.push_back(i);
        typeOverflow = 0;
      }
      else if(typeOverflow > 0)
        typeOverflow = 0;
...
void Executor::writeTestCase(ExecutionState &state, const char* message, const char* error) {
  if (!OnlyOutputStatesCoveringNew || state.coveredNew ||
      (AlwaysOutputSeeds && seedMap.count(&state)))
    interpreterHandler->processTestCase(state, message, error);
  // terminateState(state);
}
//-------------------------------
...
void Executor::executeMemoryOperation(ExecutionState &state,
  ...
  if (unbound) {
    if (incomplete) {
        ...
        //   outs() << "inst:" << *i << '\n';
        //   // if(i->getFunction())
        //   // outs() << "func:" << i->getFunction()->getName() << '\n';
        // }
...
void Executor::printValue(Value* v)
{
    v->printAsOperand(outs(), true, nullptr);
    outs() << "\n";
}

void Executor::printVector(std::vector<Value*> vec)
{
...
void Executor::printList(std::list<Value*> vec)
{
    for(auto itr = vec.begin(); itr != vec.end(); ++itr){
        printValue(*itr);
    }
...
int Executor::checkStateUseBranch(Function* f, std::queue<BasicBlock*> q, Instruction* sendInst)
{
    int first = 0;
    std::list<Value*> classVariables = allClassVariables[f];

    while(!q.empty()){
        BasicBlock* currBlock = q.front();
        q.pop();
        BasicBlock::iterator inst = currBlock->begin();
        for (BasicBlock::iterator instEnd = currBlock->end(); inst != instEnd; ++inst){
            for (Use &U : inst->operands()) {
                Value *operand = U.get();
                if(std::find(classVariables.begin(), classVariables.end(), operand) != classVariables.end()){
                    return 1;
                }
            }
        }
        Instruction* last = currBlock->getTerminator();
        if(BranchInst *br = dyn_cast<BranchInst>(last)){
            unsigned countSucc = br->getNumSuccessors();
            for(unsigned i = 0; i < countSucc; ++i){
...
int Executor::checkStateUse(Function* f, BasicBlock* sendBlock, Instruction* inst)
{
    int retVal = 0;
    BasicBlock *branch1, *branch2;
    Instruction* last = sendBlock->getTerminator();
    unsigned branches;
    if(BranchInst *br = dyn_cast<BranchInst>(last)){
        branches = br->getNumSuccessors();
        std::queue<BasicBlock*> queue1;
        // outs() << "branch:" << branches << "\n";
        if(branches > 0){
            branch1 = br->getSuccessor(0);
            queue1.push(branch1);
            retVal = checkStateUseBranch(f, queue1, inst);
            // outs() << "branch1:" << retVal << "\n";

            if(branches == 2){
                branch2 = br->getSuccessor(1);
                std::queue<BasicBlock*> queue2;
                queue2.push(branch2);
                retVal += checkStateUseBranch(f, queue2, inst);
                // outs() << "branch2:" << retVal << "\n";
            }
        }
    }
...
bool Executor::checkUncheckedSend(Function* F, ExecutionState& state)
{
    //state change after failed send.
    bool callFlag = false;
    bool fallback = false;
    bool checkSend = false;
    int stateUse = 0;

    if(newContract.hasFallback)
        fallback = true;

    // std::vector<FunctionData*> contractFunctions = contract.getFunctions();
    BasicBlock* sendBlock;
    Instruction* sendInst;
    std::vector<Value*> sendInstUsers;
    ...
    for (Function::iterator block = F->begin(), blockEnd = F->end(); block != blockEnd; ++block){
        for (BasicBlock::iterator inst = block->begin(), instEnd = block->end(); inst != instEnd; ++inst){
            if (CallInst *call = dyn_cast<CallInst>(inst)){
                ...
                if(checkTransferFunc(calledFunc)){
                    callFlag = true;
                    //check for whether if this call inst is protected by condition, get condition and check if it can be true
                    sendBlock = dyn_cast<BasicBlock>(block);
                    sendInst = dyn_cast<Instruction>(call);
                    sendInstUsers.push_back((Value*)sendInst);
                    for(auto u: ((Value*)sendInst)->users()){
                		if(Instruction *i = dyn_cast<Instruction>(u)){
                            if(std::find(sendInstUsers.begin(), sendInstUsers.end(), (Value*)(i)) == sendInstUsers.end())
                                sendInstUsers.push_back((Value*)i);
                        }
                    }
                }
            }
            ...
            if(callFlag){
                if(BranchInst *br = dyn_cast<BranchInst>(inst)){
                    if(br->isConditional()){
                        ...
                        if(std::find(sendInstUsers.begin(), sendInstUsers.end(), condition) != sendInstUsers.end()){
                            checkSend = true;
                            stateUse = checkStateUse(F, sendBlock, sendInst);
                            // outs() << "stateUse:" << stateUse;
                        }
                    }
                }
                if(Instruction *ins = dyn_cast<Instruction>(inst)){
                    if(std::find(sendInstUsers.begin(), sendInstUsers.end(), (Value*)(ins)) == sendInstUsers.end()){
                         for(auto &user1: ins->operands()){
                            Value* v1 = user1.get();
                            if(std::find(sendInstUsers.begin(), sendInstUsers.end(), v1) != sendInstUsers.end()){
                                // outs() << "inst2:" << *ins << "\n";
                                if(std::find(sendInstUsers.begin(), sendInstUsers.end(), ins) == sendInstUsers.end()){
                                    // outs() << "inst3:" << *ins << "\n";
                                    if(Value* v = cast<Value>(ins)){
                                        if(StoreInst* st = dyn_cast<StoreInst>(ins)){
                                            // for(auto &user2: ins->operands()){
                                            //     Value* v2 = user2.get();
                                            if(std::find(sendInstUsers.begin(), sendInstUsers.end(), ins->getOperand(1)) == sendInstUsers.end()){
                                                sendInstUsers.push_back((Value*)ins->getOperand(1));
                                            }
                                        }
                                        else
                                            sendInstUsers.push_back(v);
                                    }
                                }
                            }
                        }
    ...
    // if(callFlag && !fallback){
    //     os << "Transfer function fails due to no fallback: " << getFunctionName(F) << " (";
    // }
    if(!checkSend){
        os << "Unchecked Send: " << getFunctionName(F) << " (";
        buggyFunctions.push_back(F);    
        ...
        os << ") at " << state.pc->getSourceLocation();
        klee::contractError("%s", os.str().c_str());
        failProperty << "Unchecked send\n";
    }
    return checkSend;
}

void Executor::checkTOD()
{
    std::map<std::string, std::set<unsigned>> readVar = useDefObj->getReadState();
    std::map<std::string, std::set<unsigned>> writeVar = useDefObj->getWriteState();
    std::map<int, std::set<std::pair<std::string, std::string>>> conflictRW;
    std::map<int, std::set<std::pair<std::string, std::string>>> conflictWW;
    std::map<int, std::set<std::string>> read;
    std::map<int, std::set<std::string>> write;

    for(auto itr = readVar.begin(); itr != readVar.end(); ++itr){
        for(auto index = (itr->second).begin(); index != (itr->second).end(); ++index){
			// outs() << "Read: " << itr->first << ":" << *index << "\n";
            read[*index].insert(itr->first);
        }
    }
    for(auto itr = writeVar.begin(); itr != writeVar.end(); ++itr){
        for(auto index = (itr->second).begin(); index != (itr->second).end(); ++index){
			// outs() << "Write: " << itr->first << ":" << *index << "\n";
            write[*index].insert(itr->first);
        }
    }

    for(auto itr = read.begin(); itr != read.end(); ++itr){
        for(auto func1 = (itr->second).begin(); func1 != (itr->second).end(); ++func1){
            for(auto func2 = write[itr->first].begin(); func2 != write[itr->first].end(); ++func2){
                if(*func1 != *func2){
                    // outs() << "conflict:" << itr->first << '\n';
                    conflictRW[itr->first].insert(make_pair(*func1, *func2));
                }
            }
        }
    }
    for(auto itr = write.begin(); itr != write.end(); ++itr){
        for(auto func1 = (itr->second).begin(); func1 != (itr->second).end(); ++func1){
            for(auto func2 = write[itr->first].begin(); func2 != write[itr->first].end(); ++func2){
                if(*func1 != *func2){
                    // outs() << "conflict:" << itr->first << '\n';
                    conflictWW[itr->first].insert(make_pair(*func1, *func2));
                }
            }
        }
    }
    if(!conflictRW.empty() || !conflictWW.empty()){
        std::string TmpStr;
        llvm::raw_string_ostream os(TmpStr);
        os << "Transaction Order Dependence, reporting conflicting function pair:\n";
        failProperty << "Transaction Order Dependence\n";
        if(!conflictRW.empty()){
            std::vector<std::pair<std::string, std::string>> stateVar = newContract.getStateVariables();

            for(auto itr = conflictRW.begin(); itr != conflictRW.end(); ++itr){
                for(auto itr2 = (itr->second).begin(); itr2 != (itr->second).end(); ++itr2){
                    //statevar 0 = struct address
                    //edited: stateVar: 0 = address
                    if(itr->first <= stateVar.size()){
                        if(std::find(contractFunctions.begin(), contractFunctions.end(), itr2->first) != contractFunctions.end() &&
                        std::find(contractFunctions.begin(), contractFunctions.end(), itr2->second) != contractFunctions.end()){
                            //edited: stateVar: 0 = address
                            if(itr->first != 0)
                                os << "StateVariable: " << stateVar[itr->first - 1].first << ", ";
                            os << "ReadBy: " << itr2->first << ", WriteBy: " << itr2->second << "\n";
                        }
                    }
                }
            }
            // klee::contractWarning("%s", os.str().c_str());
        }
        if(!conflictWW.empty()){
            std::vector<std::pair<std::string, std::string>> stateVar = newContract.getStateVariables();
            // std::string TmpStr;
            ...
            for(auto itr = conflictWW.begin(); itr != conflictWW.end(); ++itr){
                for(auto itr2 = (itr->second).begin(); itr2 != (itr->second).end(); ++itr2){
                    //edited: stateVar: 0 = address
                    if(itr->first <= stateVar.size()){
                        if(std::find(contractFunctions.begin(), contractFunctions.end(), itr2->first) != contractFunctions.end() &&
                        std::find(contractFunctions.begin(), contractFunctions.end(), itr2->second) != contractFunctions.end()){
                            ...
                            if(itr->first != 0)
                                os << "StateVariable: " << stateVar[itr->first - 1].first << ", ";
                            os << "WriteBy: " << itr2->first << ", WriteBy: " << itr2->second << "\n";
                        }
                    }
                }
            }
...
void Executor::checkGreedy(Function* F, ExecutionState& state, std::vector< ref<Expr> > &arguments)
{
    bool callFlag = false;
    bool payable = false;
    bool fallback = false;

    std::vector<FunctionData*> contractFunc = newContract.getFunctions();
    ...
    std::string currFunc = getFunctionName(F);

    for(auto itr = contractFunc.begin(); itr != contractFunc.end(); ++itr){
        FunctionData* f = *itr;
        if(currFunc == f->getName()){
            if(f->isPayable){
                payable = true;
            }
            break;
    ...
    if(!callFlag && payable && fallback){
        failProperty << "Greedy\n";
        std::string TmpStr;
        llvm::raw_string_ostream os(TmpStr);
        os << "Contract is greedy: " << getFunctionName(F) << " (";
        for (unsigned i=0; i<arguments.size(); i++) {
        os << arguments[i];
        if (i != arguments.size()-1)
          os << ", ";
        ...
        klee::contractError("%s", os.str().c_str());
        buggyFunctions.push_back(F);
        // Executor::terminateStateOnError(state, "Greedy", Executor::User);
        return;
    }
}

void Executor::checkProdigal(Function* F, ExecutionState& state, std::vector< ref<Expr> > &arguments)
{
// todo: there may be check for local address or func arg address which finds it in state variables
    bool callFlag = false;
    bool prodigalFlag = false;
    std::vector<std::string> addrArg;

    for(Function::arg_iterator itr = F->arg_begin(); itr != F->arg_end(); itr++){
        if (StructType *IT = dyn_cast<StructType>(((Value*)itr)->getType())){
            if(itr->hasName())
                addrArg.push_back(((Value*)itr)->getName().str());
        }
    }
    for (Function::iterator block = F->begin(), blockEnd = F->end(); block != blockEnd; ++block){
        for (BasicBlock::iterator inst = block->begin(), instEnd = block->end(); inst != instEnd; ++inst){
            if (CallInst *call = dyn_cast<CallInst>(inst)){
                ...
                if(checkTransferFunc(calledFunc)){
                    ...
                    if(t->isPointerTy()){
                        ...
                        if (StructType *IT = dyn_cast<StructType>(t2)){
                        //send address in function arguments
                            for(auto itr = addrArg.begin(); itr != addrArg.end(); ++itr){
                                if (val->hasName() && val->getName().str() == *itr){
                              //klee::klee_warning_once(val, "Contract is prodigal:");
                                    prodigalFlag = true;
                                    goto warn;
                          	    }
                            }
                            //send address as local variable
                            if(AllocaInst* inst = dyn_cast<AllocaInst>(val)){
                                prodigalFlag = true;
                            // errs() << "local" << "\n";
                                goto warn;
                            }
    ...
    warn:
    if(prodigalFlag == true){
        failProperty << "Prodigal\n";
        std::string TmpStr;
        llvm::raw_string_ostream os(TmpStr);
        os << "Contract is Prodigal: " << getFunctionName(F) << " (";
        for (unsigned i = 1; i < arguments.size(); i++) {
            os << arguments[i];
            if (i != arguments.size()-1)
            os << ", ";
        ...
        os << ") at " << state.pc->getSourceLocation();
        klee::contractError("%s", os.str().c_str());
        // terminateStateOnError(state, "Prodigal", User);
        interpreterHandler->processTestCase(state, os.str().c_str(), "Prodigal");
        interpreterHandler->incPathsExplored();
        buggyFunctions.push_back(F);
...
int Executor::checkSuicidalFunc(Function* currFunc, CallInst* callInst, Function* calledFunc)
{
    int retVal = 0;
    std::string funcName = getFunctionName(calledFunc);
    std::list<Value*> classVariables = allClassVariables[currFunc];

    Value* global = NULL;
    if(globalVariables.find("msgSender") != globalVariables.end()){
        global = globalVariables["msgSender"];
    }
    std::list<Value*> g;
    g.push_back(global);

    if(funcName == "suicide" || funcName == "selfdestruct"){
        retVal = 1;
        std::map<Value*, Value*> temp;
        std::list<Value*> msgUsers = useDefObj->getAllUsers(currFunc, g, temp);
        // outs() << "global:";
        // for(auto itr = msgUsers.begin(); itr != msgUsers.end(); ++itr){
        //     (*itr)->printAsOperand(outs(), true, nullptr);
        //     outs() << "\n";
        // }
        // outs() << "arg:";
        Value* arg = (Value*)(callInst->getArgOperand(0));
        // (arg)->printAsOperand(outs(), true, nullptr);
        // outs() << "\n";
        if(global != NULL && std::find(msgUsers.begin(), msgUsers.end(), arg) != msgUsers.end()){
            retVal = 2;
        }
        else {
            if(!classVariables.empty()){
                // classVariables = useDefObj->getClassVariables(currFunc);
                if(std::find(classVariables.begin(), classVariables.end(), arg) != classVariables.end()){
                    retVal = 3;
                }
            }
        }
...
void Executor::checkSuicidal(Function* F, ExecutionState& state, std::vector< ref<Expr> > &arguments)
{
    bool callFlag = false;
    int suicideFlag = 0;
    // getDebugVar(F);
    ...
    for (Function::iterator block = F->begin(), blockEnd = F->end(); block != blockEnd; ++block){
      for (BasicBlock::iterator inst = block->begin(), instEnd = block->end(); inst != instEnd; ++inst){
        if (CallInst *call = dyn_cast<CallInst>(inst)){
          Function* calledFunc = call->getCalledFunction();
          if(!callFlag)
            callFlag = checkTransferFunc(calledFunc);
          if(!suicideFlag)
             suicideFlag = checkSuicidalFunc(F, call, calledFunc);
    ...
    warn:
    if(suicideFlag > 0){
      failProperty << "Suicidal\n";
      std::string TmpStr;
      llvm::raw_string_ostream os(TmpStr);

      os << "Contract is Suicidal" ;
      if(callFlag)
        os << ", containing function transacts, ";
      else
        os << ", containing function do not transact, ";

      if(suicideFlag == 2){
          os << "receipient might be msg.sender, ";
      }
      else if(suicideFlag == 3){
          os << "receipient might be state variable, ";
      }
      else{
          os << "receipient might be an arbitrary address, ";
      }
      os << getFunctionName(F) << " (";
      for (unsigned i = 1; i < arguments.size(); i++) {
        os << arguments[i];
        if (i != arguments.size()-1)
          os << ", ";
...
void Executor::checkTxOrigin(Instruction *currInst, ExecutionState& state)
{
    if(globalVariables.find("txOrigin") != globalVariables.end()){
        Value* global = globalVariables["txOrigin"];

        if(global != NULL){
            for(auto& u: currInst->operands()){
                Value* v = u.get();
                if(global == v){
                    Function* f = currInst->getFunction();
                    if(! f->hasInternalLinkage()){
                        // outs() << getFunctionName(f) << "\n";
                        failProperty << "Transaction State Dependence\n";
                        std::string TmpStr;
                        llvm::raw_string_ostream os(TmpStr);
                        os << "tx.origin should not be used: " << getFunctionName(f);
                        os << "(";
                        os << ") at ";
                        os << state.pc->getSourceLocation();
                        klee::contractWarning("%s", os.str().c_str());
                        buggyInst.push_back(currInst);
                        break;
                    }
                }
            }
...
void Executor::checkBlockhash(Instruction *currInst, ExecutionState& state)
{
    if(std::find(buggyInst.begin(), buggyInst.end(), currInst) == buggyInst.end()){

        Value* global = NULL;
        if(globalVariables.find("blockNumber") != globalVariables.end()){
            global = globalVariables["blockNumber"];
        }
        ...
        if(global != NULL){
            ...
            // std::string TmpStr;
            // llvm::raw_string_ostream os(TmpStr);
            // os << "Transaction State Dependence, block.number: " << getFunctionName(currFunc);
            // os << "(";
            // os << ")";
            // klee::contractWarning("%s", os.str().c_str());
            ...
            if(CallInst *call = dyn_cast<CallInst>(currInst)){
                ...
                if(f != NULL && getFunctionName(f) == "blockhash"){
                    std::map<Value*, Value*> temp;
                    std::list<Value*> allUsers = useDefObj->getAllUsers(currFunc, g, temp);
                    if(call->getNumOperands() > 0){
                        if(std::find(allUsers.begin(), allUsers.end(), (Value*)(call->getArgOperand(0))) != allUsers.end()){
                            failProperty << "Block State Dependence\n";
                            std::string TmpStr;
                            llvm::raw_string_ostream os(TmpStr);
                            os << "blockhash(block.number) always return 0: " << getFunctionName(currFunc);
                            os << "(";
                            os << ") at ";
                            os << state.pc->getSourceLocation();
                            klee::contractWarning("%s", os.str().c_str());
...
void Executor::checkTimestamp(Instruction *currInst, ExecutionState& state)
{
    Value* global = NULL;
    if(globalVariables.find("blockTimestamp") != globalVariables.end()){
        global = globalVariables["blockTimestamp"];
            ...
            for(auto& u: currInst->operands()){
                ...
                if(global == v){
                    ...
                    if(! f->hasInternalLinkage()){
                        ...
                        std::string TmpStr;
                        llvm::raw_string_ostream os(TmpStr);
                        os << "TimeStamp Dependence: " << getFunctionName(f);
                        os << "(";
                        os << ") at ";
                        os << state.pc->getSourceLocation();
                        klee::contractWarning("%s", os.str().c_str());
...
void Executor::checkFallback()
{
    std::string message;
    llvm::raw_string_ostream os(message);
    // os << state.pc->getSourceLocation();
    bool payableFlag = false;

    if(!newContract.hasFallback){
        for(unsigned i = 0; i < newContract.functions.size(); ++i){
            if(newContract.functions[i]->isPayable){
                os << newContract.functions[i]->name << " is payable, ";
                payableFlag = true;
            }
        }
    }
    if(payableFlag){
        os << "contract dont have fallback, should not be accepting payment" << '\n';
        klee::contractWarning("%s", os.str().c_str());
        failProperty << "Payable but no fallback\n";
    }
}

std::string Executor::getDebugVar(Instruction* inst/*Function* Caller*/)
{
...
std::string Executor::getFunctionName(Function* f)
{
    if(f != NULL){
...
bool Executor::checkTransferFunc(Function* Caller)
{
    if(Caller != NULL && !Caller->isIntrinsic()){
        if(DISubprogram *di = Caller->getSubprogram()){
            std::string name = di->getName();
            if(name == "send" || name == "call" || name == "transfer" || name == "staticcall" || name == "delegatecall"){
                return true;
            }
        }
    }
...
void Executor::checkTransfer(Instruction* currInst, ExecutionState& state){

    if(CallInst* call = dyn_cast<CallInst>(currInst)){
        Function* f = call->getCalledFunction();
        if(checkTransferFunc(f)){
            // if(newContract.hasFallback){
            std::string TmpStr;
            llvm::raw_string_ostream os(TmpStr);
            os << "Transfer function invokes fallback of other contract: " << getFunctionName(f);
            os << "(";
            os << ") at ";
            os << state.pc->getSourceLocation();
            klee::contractWarning("%s", os.str().c_str());
...
void Executor::checkMapRead(Function* currFunc, ExecutionState& state)
{
    std::map<std::string, std::set<unsigned>> readVar = useDefObj->getReadState();
    std::vector<std::pair<std::string, std::string>> stateVariables = newContract.getStateVariables();

    //check ordering of variables
    std::string funcName = getFunctionName(currFunc);

    if(readVar.find(funcName) != readVar.end()){
        for(auto itr = readVar[funcName].begin(); itr != readVar[funcName].end(); ++itr){
            //edited: stateVar: 0 = address
            if(*itr <= stateVariables.size()){
                //edited: stateVar: 0 = address
                if(*itr != 0){
                    if((stateVariables[*itr - 1].second).find("map") != std::string::npos){
                        failProperty << "Mapping read\n";
                        std::string TmpStr;
                        llvm::raw_string_ostream os(TmpStr);
                        os << "Reading mappings may return 0 if key is not present, when not valid check explicitly for value 0:" << stateVariables[*itr - 1].first << ": " << funcName;
                        os << "(";
                        os << ") at ";
                        os << state.pc->getSourceLocation();
                        klee::contractWarning("%s", os.str().c_str());
                        buggyFunctions.push_back(currFunc);
                    }
                }
            }
        }
...
void Executor::checkOverflow(Instruction* currInst, ExecutionState& state)
{
    ...
    if(std::find(buggyInst.begin(), buggyInst.end(), currInst) == buggyInst.end()){
        if(currFunc != NULL){
            std::string currFuncName = getFunctionName(currFunc);
            if( !currFunc->isDeclaration() && !currFunc->isIntrinsic()){
                std::string TmpStr;
                llvm::raw_string_ostream os(TmpStr);
                if(CallInst *call = dyn_cast<CallInst>(currInst)){
                    Function* f = call->getCalledFunction();
                    if(f != NULL && f->isIntrinsic()){
                        ...
                        if(funcName.find("ubsan_handle_add_overflow") != std::string::npos){
                            overflowFlag = true;
                            os << "Addition Overflow(unsigned): ";
                        }
                        else if(funcName.find("llvm.sadd.with.overflow") != std::string::npos){
                            overflowFlag = true;
                            os << "Addition Overflow(signed): ";
                        }
                        else if(funcName.find("llvm.ssub.with.overflow") != std::string::npos){
                            overflowFlag = true;
                            os << "Subtraction Overflow(signed): ";
                        }
                        else if(funcName.find("llvm.usub.with.overflow") != std::string::npos){
                            overflowFlag = true;
                            os << "Subtraction Overflow(unsigned): ";
                        }
                        else if(funcName.find("llvm.smul.with.overflow") != std::string::npos){
                            overflowFlag = true;
                            os << "Multiplication Overflow(signed): ";
                        }
                        else if(funcName.find("llvm.umul.with.overflow") != std::string::npos){
                            overflowFlag = true;
                            os << "Mutiplication Overflow(unsigned): ";
                        }
                    }
                    if(overflowFlag){
                        failProperty << "Overflow\n";
                        os << currFunc << "() at ";
                        os << state.pc->getSourceLocation();
                        klee::contractError("%s", os.str().c_str());
                        buggyInst.push_back(currInst);
...
void Executor::checkAddressTypecast(Instruction* i, Function* currFunc, Function* targetFunc, ExecutionState& state){
  if(currFunc && targetFunc && std::find(buggyInst.begin(), buggyInst.end(), i) == buggyInst.end()){
    if(CallInst* call = dyn_cast<CallInst>(i)){
      std::string name = getFunctionName(targetFunc);
      std::string nameArg = targetFunc->getName().str();

      std::vector<std::string> contractNames = readContractNames();
      if(std::find(contractNames.begin(), contractNames.end(), name) != contractNames.end()){
        if(nameArg.find("address") != std::string::npos && call->getNumArgOperands() == 2){
          // outs() << call->getNumArgOperands() << '\n';   
          failProperty << "Address Typecast\n"; 
          std::string TmpStr;
          llvm::raw_string_ostream os(TmpStr);
          os << "Address Typecast " << name << ": " << getFunctionName(currFunc);
          os << "() at ";
          os << state.pc->getSourceLocation();
          klee::contractWarning("%s", os.str().c_str());
          buggyInst.push_back(call);
        }
      }
    }
  }

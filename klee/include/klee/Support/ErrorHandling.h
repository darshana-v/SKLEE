...
void contractWarning(const char *msg, ...) __attribute__((format(printf, 1, 2)));
void contractError(const char *msg, ...) __attribute__((format(printf, 1, 2)));
void overflowWarning(const char* msg, ...) __attribute__((format(printf, 1, 2)));

void setOverflow(unsigned type);

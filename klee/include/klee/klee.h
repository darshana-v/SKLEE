...
extern "C" {
   ...
   */
    ...
    */
    ...
    void overflowWarning(const char* message);
  ...
  /* Get errno value of the current state */
    ...
    //
    // // returns (bytes32): hash of the given block - only works for 256 most recent blocks
    // char* blockhash(unsigned int blockNumber);
    //
    // // returns (bytes32): compute the Keccak-256 hash of the input
    // char* keccak256(/*bytes*/char* memory);
    //
    // // returns (bytes32): compute the SHA-256 hash of the input
    // char* sha256(/*bytes*/char* memory);
    //
    // // returns (bytes20): compute the RIPEMD-160 hash of the input
    // char* ripemd160(/*bytes*/char* memory);
    //
    // // returns (address): recover address associated with the public key from elliptic curve signature, return zero on error
    // address ecrecover(/*bytes32*/char* hash, unsigned int /*uint8*/ v, char* /*bytes32*/ r, char* /*bytes32*/ s);
    //
    // // returns (uint): compute (x + y) % k where the addition is performed with arbitrary precision and does not wrap around at 2**256. Assert that k != 0 starting from version 0.5.0.
    // unsigned int addmod(unsigned int x, unsigned int y, unsigned int k);
    //
    // // returns (uint): compute (x * y) % k where the multiplication is performed with arbitrary precision and does not wrap around at 2**256. Assert that k != 0 starting from version 0.5.0.
    // unsigned int mulmod(unsigned int x, unsigned int y, unsigned int k);
    //
    // // destroy the current contract, sending its funds to the given address
    // int selfdestruct(address /*payable*/ recipient);
    //
    // int suicide(address recipient);
